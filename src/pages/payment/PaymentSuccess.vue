<template>
  ...
  <div v-else-if="paymentData" class="flex flex-col items-center bg-white w-full h-screen">
    ...
    <!-- 완료 메시지 -->
    <p class="mt-[60px] text-[20px] text-black font-normal">결제가 완료되었습니다!</p>

    <!-- 버튼 -->
    <div class="mt-[329px] px-[20px] w-full">
      <BaseButton variant="primary" size="lg" @click="goToHome" class="w-full max-w-[353px] mx-auto">
        돌아가기
      </BaseButton>
    </div>
  </div>
  ...
  <div v-else class="flex flex-col items-center justify-center w-full h-screen bg-white text-center px-4">
    <h1 class="text-[#dc3545] text-[18px] mt-[100px] font-semibold">
      {{ error || '결제 처리 중 오류가 발생했습니다' }}
    </h1>
    <div class="mt-[40px] px-[20px] w-full">
      <BaseButton variant="primary" size="lg" @click="goToHome" class="w-full max-w-[353px] mx-auto">
        돌아가기
      </BaseButton>
    </div>
