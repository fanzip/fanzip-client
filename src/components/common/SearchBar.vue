  ...
  <div class="flex items-center w-full max-w-[393px] px-4 py-2">
    ...
    <div class="relative flex items-center w-[353px] h-[32px] bg-subtle-bg rounded-[16px]">
      <input
        ...
        class="w-full h-full text-[16px] placeholder-text-subtle-text rounded-[16px] outline-none bg-transparent pl-[16px] pr-[36px]"
      />

      <!-- 돋보기 아이콘 or X 버튼 -->
      <template v-if="!keyword">
        <img src="@/assets/searchbar/search.svg" alt="검색" class="absolute right-[16px] w-4 h-4" />
      </template>
      <template v-else>
        <button @click="clearKeyword" class="absolute right-[16px] w-4 h-4">
