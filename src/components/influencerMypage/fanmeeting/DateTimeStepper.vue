...
const props = defineProps({
  ...
  minuteStep: { type: Number, default: 10 }, // 5/10/30 등 가능
})
...
function next() {
  if (selDate.value) step.value = 'time'
}
function back() {
  step.value = 'date'
}
...
<template>
  ...
  <button
    type="button"
    class="w-full h-11 px-4 rounded-xl border border-base-border bg-base-bg text-sm text-left"
    @click="openSheet"
  >
