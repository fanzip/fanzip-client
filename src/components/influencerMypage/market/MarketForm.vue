        ...
        <template v-if="selectedCategories.length">
          <template v-for="name in selectedCategories" :key="name">
            ...
            <img
              ...
              @click.stop="selectedCategories = selectedCategories.filter((n) => n !== name)"
            />
          </template>
        </template>
    ...
    <div class="w-[calc(100%-40px)] max-w-sm bg-base-bg rounded-xl shadow-xl p-4">
      ...
      <div class="grid grid-cols-3 gap-2">
        <button
          ...
          "
          @click="
            selectedCategories = selectedCategories.includes(cat.ko)
              ? selectedCategories.filter((n) => n !== cat.ko)
              : [...selectedCategories, cat.ko]
          "
        >
          {{ cat.ko }}
        </button>
