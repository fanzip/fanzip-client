<template>
  <div class="flex items-center w-full max-w-[393px] px-4 py-2">
    ...
    <div class="relative flex items-center w-[353px] h-8 bg-subtle-bg rounded-2xl">
      <input
        v-model="keyword"
        :placeholder="placeholder"
        class="w-full h-full text-base text-[#504E4E] rounded-2xl outline-none bg-transparent pl-4 pr-9"
      />

      <!-- 돋보기 아이콘 or X 버튼 -->
      <template v-if="!keyword">
        <img src="@/assets/searchbar/search.svg" alt="검색" class="absolute right-4 w-4 h-4" />
      </template>
      <template v-else>
        <button @click="clearKeyword" class="absolute right-4 w-4 h-4">
